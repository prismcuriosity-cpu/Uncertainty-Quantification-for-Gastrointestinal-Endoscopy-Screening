"""Script to build the complete Kaggle notebook JSON."""
import json
...
# ─── Cell 2: Quick/Full switch ────────────────────────────────────────────────
cells.append(md("## 2. Quick / Full Run Switch\n"
...
# ─── Cell 4: Class names ──────────────────────────────────────────────────────
cells.append(md("## 4. Class Names & Mapping"))
cells.append(code(
"# ── 4. Class Names & Mapping ──────────────────────────────────────────────\n"
"# Expected class folder names mapped to integer labels.\n"
"# Edit CLASS_MAP if your folder names differ from the defaults below.\n"
"CLASS_MAP = {\n"
"    'GERD':        0,\n"
"    'GERD-Normal': 1,\n"
"    'Polyp':       2,\n"
...
# ─── Cell 5: Dataset scanning ─────────────────────────────────────────────────
cells.append(md("## 5. Dataset Scanning\n"
...
# ─── Cell 6: Dataset visualizations ──────────────────────────────────────────
cells.append(md("## 6. Dataset Visualizations"))
cells.append(code(
...
"    if quick_run:\n"
"        df_prim = (\n"
"            df_prim.groupby('class_name', group_keys=False)\n"
"                   .apply(lambda g: g.sample(\n"
"                       min(cfg.QUICK_MAX_PER_CLASS, len(g)), random_state=seed))\n"
"                   .reset_index(drop=True)\n"
"        )\n"
"        print(f'Quick mode: {len(df_prim)} primary images sampled')\n"
"\n"
"    pids   = df_prim['primary_id'].values\n"
"    labels = df_prim['label'].values\n"
"\n"
"    # Step 1: carve test+cal from full set\n"
"    test_cal_frac = cfg.TEST_FRAC + cfg.CAL_FRAC\n"
"    n_splits_tc   = max(2, round(1 / test_cal_frac))\n"
"    sgkf = StratifiedGroupKFold(n_splits=n_splits_tc, shuffle=True, random_state=seed)\n"
"    train_val_idx, test_cal_idx = next(iter(sgkf.split(pids, labels, pids)))\n"
"    df_trainval = df_prim.iloc[train_val_idx].reset_index(drop=True)\n"
"    df_testcal  = df_prim.iloc[test_cal_idx].reset_index(drop=True)\n"
"\n"
"    # Step 2: split test_cal into test and cal\n"
"    cal_frac_of_tc = cfg.CAL_FRAC / test_cal_frac\n"
"    sgkf2 = StratifiedGroupKFold(\n"
"        n_splits=max(2, round(1 / cal_frac_of_tc)), shuffle=True, random_state=seed+1)\n"
"    cal_idx, test_idx = next(iter(sgkf2.split(\n"
"        df_testcal['primary_id'].values,\n"
"        df_testcal['label'].values,\n"
"        df_testcal['primary_id'].values)))\n"
"    df_cal  = df_testcal.iloc[cal_idx].reset_index(drop=True)\n"
"    df_test = df_testcal.iloc[test_idx].reset_index(drop=True)\n"
"\n"
"    # Step 3: split train_val into train and val\n"
"    val_frac_of_tv = cfg.VAL_FRAC / (cfg.TRAIN_FRAC + cfg.VAL_FRAC)\n"
"    sgkf3 = StratifiedGroupKFold(\n"
"        n_splits=max(2, round(1 / val_frac_of_tv)), shuffle=True, random_state=seed+2)\n"
"    train_idx, val_idx = next(iter(sgkf3.split(\n"
"        df_trainval['primary_id'].values,\n"
"        df_trainval['label'].values,\n"
"        df_trainval['primary_id'].values)))\n"
"    df_train = df_trainval.iloc[train_idx].reset_index(drop=True)\n"
"    df_val   = df_trainval.iloc[val_idx].reset_index(drop=True)\n"
"\n"
"    return df_train, df_val, df_cal, df_test\n"
"\n"
"df_prim_train, df_prim_val, df_prim_cal, df_prim_test = make_splits(\n"
...
"\n"
"if QUICK_RUN:\n"
"    all_sampled_pids = (set(df_prim_train['primary_id']) | set(df_prim_val['primary_id'])\n"
"                      | set(df_prim_cal['primary_id'])   | set(df_prim_test['primary_id']))\n"
"    df_use = df_all[df_all['primary_id'].isin(all_sampled_pids)]\n"
"else:\n"
"    df_use = df_all\n"
"\n"
...
"    if overlap:\n"
"        print(f'LEAKAGE between {a} and {b}: {len(overlap)} PIDs overlap!')\n"
"        leak_found = True\n"
"if leak_found:\n"
...
"\n"
"for name, split_df in [('train',df_train),('val',df_val),('cal',df_cal),('test',df_test)]:\n"
"    counts = split_df.groupby('class_name').size()\n"
"    print(f'  {name:6s}: {len(split_df):5d} images | '\n"
"          + ' | '.join(f'{c}:{n}' for c,n in counts.items()))\n"
"\n"
"df_train_s = df_train.copy(); df_train_s['split'] = 'train'\n"
"df_val_s   = df_val.copy();   df_val_s['split']   = 'val'\n"
"df_cal_s   = df_cal.copy();   df_cal_s['split']   = 'cal'\n"
"df_test_s  = df_test.copy();  df_test_s['split']  = 'test'\n"
"df_split   = pd.concat([df_train_s, df_val_s, df_cal_s, df_test_s], ignore_index=True)\n"
"split_path = f'{OUTPUT_DIR}/tables/split_file.csv'\n"
"df_split.to_csv(split_path, index=False)\n"
"print(f'\\nSplit file saved -> {split_path}')\n"
))
...
# ─── Cell 9: Models ─────────────────────────────────────────────────────────
cells.append(md("## 9. Model Definitions\n"
"ConvNeXt-Tiny and ViT-S/16 from `timm` with 4-class head. "
"Falls back to random init if pretrained weights cannot download."))
cells.append(code(
"# ── 9. Models ─────────────────────────────────────────────────────────────\n"
"import timm\n"
...
"AdamW with separate LRs for backbone/head, mixed precision, cosine LR, "
"early stopping, and auto-OOM batch-size halving."))
cells.append(code(
"# ── 10. Training ──────────────────────────────────────────────────────────\n"
"import time, copy\n"
...
"\n"
"Temperature scaling is fitted on the **calibration split** exclusively. "
"Evaluation is then performed on the **test split** to avoid leakage.\n"
))
cells.append(code(
"# ── 12. Calibration ───────────────────────────────────────────────────────\n"
...
# ─── Cell 14: Selective classification ────────────────────────────────────────
cells.append(md("## 14. Selective Classification / Abstention"))
cells.append(code(
...
# ─── Cell 15: Journal figures ─────────────────────────────────────────────────
cells.append(md("## 15. Journal-Ready Figures"))
cells.append(code(
...
# ─── Cell 16: Qualitative panel ───────────────────────────────────────────────
cells.append(md("## 16. Qualitative Panel: Confident Correct / Wrong / Abstained"))
cells.append(code(
...
# ─── Cell 17: Result tables ────────────────────────────────────────────────────
cells.append(md("## 17. Journal Result Tables"))
cells.append(code(
"# ── 17. Result Tables ─────────────────────────────────────────────────────\n"
"\n"
...
# ─── Cell 18: Ensemble ────────────────────────────────────────────────────────
cells.append(md("## 18. Optional Ensemble (if ≥ 2 models trained)"))
cells.append(code(
"# ── 18. Ensemble ──────────────────────────────────────────────────────────\n"
...
# ─── Cell 19: Output packaging ────────────────────────────────────────────────
cells.append(md("## 19. Output Packaging"))
cells.append(code(
"# ── 19. Output Packaging ──────────────────────────────────────────────────\n"
...
out_path = "gi_endoscopy_conformal.ipynb"
with open(out_path, 'w') as f:
    json.dump(notebook, f, indent=1)

print(f"Notebook written: {out_path}")
print(f"Total cells: {len(cells)}")
print(f"File size: {pathlib.Path(out_path).stat().st_size / 1024:.0f} KB")
